...
public class BinaryTree
{
	...
	public void insert(boolean data, String featureName){
		root = insert(root,data, featureName);
		//root.counter=count++;
		size++;
	}

	public Node insert(Node node, boolean data, String featureName){
		if (node == null){
			node = new Node(data, featureName);
			node.counter=count++;
			size++;
		}else{
			if (node.left  == null){
				node.left = insert(node.left, data, featureName);
				node.left.counter=count++;
				size++;
			}else{
				node.right = insert(node.right,data, featureName);
				node.right.counter=count++;
				size++;
			}
		}
